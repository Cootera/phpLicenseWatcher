...
function list_licenses_in_use($servers, &$html_body) {
    ...
    foreach ($servers as $server) {
        ...
        $col_headers = array("Feature", "Total Licenses", "Details", "Time Checked Out");
        $table->add_row($col_headers, array(), "th");
        ...
        foreach(array_merge($used_licenses, $unused_licenses) as $i => $license) {
            ...
            $license_info  = "{$licenses_used} currently in use, ";
            $license_info .= array_key_exists('num_queued', $license) ? "{$license['num_queued']} queued, " : "";
            $license_info .= ($licenses_reserved > 0) ? "{$licenses_reserved} reserved, " : "";
            $license_info .= "<br><span class='bold-text'>{$licenses_available} available</span>";
                ...
                if ($licenses_used > 0) {
                  ...
                  $table->add_row(array($license['feature_name'], $license['num_licenses'], $license_info, ""), $class);
